...
return buffer
    ...
    const fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6283102650464-1589321480@g.us" } : {}) },message: { "videoMessage": { "title":`*AUTO DOWNLOAD AUDIO YOUTUBE*`, "h": `Hmm`,'seconds': '10000000⁰0', 'caption': `*AUTO DOWNLOAD AUDIO YOUTUBE*`, 'jpegThumbnail': global.thumb}}}
		...
		const buttonsDefault = [
			//{ callButton: { displayText: `Call Owner!`, phoneNumber: `+6285808936396` } },
			{ urlButton: { displayText: `github!`, url : `https://beacons.ai/chisato` } },
...
`,
					btn = [{
                                urlButton: {
                                    ...
                                    url: 'https://instagram.com/me_kyluesky'
                                }
                            }, {
                                urlButton: {
                                    displayText: 'ᴡᴇʙsɪᴛᴇ',
                                    url: 'https://beacons.ai/chisato'
                                }
                            }, {
                                quickReplyButton: {
                                    displayText: '❗ ᴄʀᴇᴅɪᴛ',
				...
				break
			case 'shsjjsc':
			case 'dlhdhjddjdxil': // biar nggak nabrak sama bot lain
			case 'scrijajbsjsnsjpt':
			case 'sourcehshhscohshde':
			case 'credit':
			case 'info':{
...
Konfirmasi pembayaran mu dengan menunjjukan bukti transfer kepada owner!
				 ...
				 but = [
                    {buttonId: `qriscrhizzy`, buttonText: {displayText: 'Pay With QRIS'}, type: 1},
			...
			case 'tk': case 'tarikkomisi': {
				...
				let nyobb = list
				conn.sendMessage(from, {text:`*PILLIH E-WALLET*\nPilih jenis wallet yang ingin anda gunakan!`, title: 'WALLET', footer: '© 2022 Nishikigi Chisato ', buttons: nyobb})
				}
				break
...
sections = [
                ...
                {title: "Script Bot", rowId: `.dldhshlowwwowwshhxil`, description: `Menampilkan Script Bot`}
                ]
                },
                ]
conn.sendListMsg(from, `Silahkan click tombol dibawah ini`, `© 2022 Nishikigi Chisato `, `Hai ${pushname} 👋`, `Click Here`, sections, fake)
      }
break
case 'nmrownxnjayxz': { // yg lain yg ada button owner ganti ke ini biar langsung ke contact
// follow https://github.com/Chandra-XD
cok = {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...({ remoteJid: "status@broadcast" })}, message: { 'contactMessage': { 'displayName': `Elaina Bot`, 'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;Crizzy Bot,;;;\nFN:Crizzy Bot,\nitem1.TEL;waid=0:0\nitem1.X-ABLabell:Ponsel\nEND:VCARD`} }}
...
┆⫹⫺ Nama : Nishikigi Chisato
┆⫹⫺ Hobby : Watching Anime
┆⫹⫺ Status : Singgle
┆⫹⫺ Birthday : Private
┆⫹⫺ Address : Japan - Iwate - Yamada
┆⫹⫺ Age : 17
└––––––––––––––––––·•
...
ngisor = `Social Media :
YouTube : Kyluesky
Instragram : @me_kyluesky
TikTok : real_keyy
Github : -
Email : kylueskynih@gmail.com
...
break*/
			...
			let buttons = [
                ...
                conn.sendMessage(from, {text:`*NGETES BUTTON*`, title: 'WALLET', footer: '© 2022 Nishikigi Chisato ', buttons: buttons})
                }
                break
			case 'request': case 'requestfitur':{
				if (!text) return m.reply(`Text mana?\n\nExample : ${prefix + command} Tambahin fitur tiktok downloader dong`)
				conn.sendMessage(`6285808936396@s.whatsapp.net`, {text: `*Hallo owner ada yang request fitur nih*\n\nPesan : ${text}\nPengirim : ${m.sender}`}, {quoted: {key: { fromMe: false,
...
if ((m.mtype === 'reactionMessage') && m.isGroup) {
    conn.sendMessage(from, {text: `Terdeteksi *${m.pushName} @${m.sender.split("@")[0]}* Mengirim Reaction.`, mentions: [m.sender]}, {quoted: fake})
        ...
        if ((m.mtype === 'groupInviteMessage' || budy.startsWith('https://chat') || budy.startsWith('Buka tautan ini')) && !m.isBaileys && !m.isGroup) {
			...
			conn.sendMessage(from, {text:`INVITE DETECT\n\nMau masukin bot ini ke group kamu?`, title: 'LINK GROUP DECTETED', footer: '© 2022 Nishikigi Chisato ', buttons: buttons, headerType: 5})
		}
